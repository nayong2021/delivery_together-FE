...
const avatarhtml = function (message) {
  ...
  if (!message.fileUrl) {
    ...
    if (message.data.fileTypeLabel === "video") {
      template = `<div class="message-list avatar">
				<div class="avatar-image"><img src="${message.profileImageUrl}" /></div>
				<div class="message-box">
					<div class="avatar-name">${message.username}</div>
					<div class="message-time">
    ...
    <div class="message-list wirter">
		<div class="message-box">
          ...
          message.createdAt
				...
				<div class="file-area picture">
					<a href="${message.fileUrl}" target="_blank"><img src="${
    message.fileUrl
  }" /></a>
				</div>
			</div>
  ...
  client.loginAnonymous(
    {
      ...
      username: avatarUser[pick].username,
      profileImageUrl: avatarUser[pick].image,
    },
...
function addMessageText(messageText) {
  client.sendMessage(
    { channelId: DEMO_CHANNEL_ID, type: "text", text: messageText },
    function (err, data) {
      if (err) {
        return alert(err);
      }
    ...
    client.sendMessage(
      {
        ...
        text: "",
        data: {
          fileSizeLabel: imgFileSize,
          fileNameLabel: imgFile.name,
          fileTypeLabel: "image",
        },
        file: imgFile,
      },
...
function sendFileVideo() {
  $(".attach-box .attach.video").on("click", function () {
    ...
    let videoFile = new File([blob], "TalkPlusSampleVideo", {
      type: "video/mp4",
    });
    ...
    client.sendMessage(
      {
        ...
        text: "",
        data: {
          fileSizeLabel: videoFileSize,
          fileNameLabel: videoFile.name,
          fileScreenShotUrl: videoUrl,
          fileTypeLabel: "video",
        },
        file: videoFile,
      },
...
function sendFileText() {
  $(".attach-box .attach.file").on("click", function () {
    let file = new File(["talkplus sample file..."], "Talkplus Sample.txt", {
      type: "text/plain",
    });
